...
const MINT_INITIAL_SUPPLY_CHAIN_ID = process.env.MINT_INITIAL_SUPPLY_CHAIN_ID
  ? +process.env.MINT_INITIAL_SUPPLY_CHAIN_ID
  : 11155111;
...
const deploy: DeployFunction = async (hre) => {
  ...
  if (hre.network.config.chainId === MINT_INITIAL_SUPPLY_CHAIN_ID) {
    console.log(
      `Deploying contract and minting initial supply with deployer: ${deployer}`,
    );

    const signer = await hre.ethers.getSigner(deployer);

    const { address, abi } = await deploy(contractNameMainnet, {
      from: deployer,
      args: [
        name,
        symbol,
    ...
    console.log(
      `Deploying contract without minting initial supply with deployer: ${deployer}`,
    );
